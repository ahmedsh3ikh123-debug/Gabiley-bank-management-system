...
import {
  ...
  User, KeyRound, Shield, ArrowRight, RefreshCcw, Users, MessageSquare,
} from "lucide-react";
...
export default function ResetAccountPage() {
  ...
  const handleRequestAdminHelp = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email && !username) { showError("Please enter your email or username"); return; }
    setLoading(true);
    try {
      await api.post("/auth/request-admin-reset", { email: email || username, reason });
      setMode("admin");
      setStep(2);
        ...
        <>
          <div className="mb-8 text-center">
            <div className="inline-flex items-center gap-2 rounded-full bg-[#1F8A4D]/10 border border-[#1F8A4D]/20 px-4 py-1.5 mb-5 animate-float">
              <RefreshCcw className="h-3.5 w-3.5 text-[#1F8A4D]" />
            ...
            <div className="animate-slide-up" style={{ animationDelay: "0.1s" }}>
              <Button type="submit" disabled={loading}
                ...
                {loading ? (
                  ...
                  <span className="relative flex items-center justify-center">Send Reset Code<ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform duration-200" /></span>
                )}
              </Button>
            </div>
          </form>
      ...
      {step === 1 && mode === "admin" && (
        <>
          ...
          <form onSubmit={handleRequestAdminHelp} className="space-y-5">
            <div className="space-y-2 animate-slide-up" style={{ animationDelay: "0.05s" }}>
              <Label htmlFor="admin-username" className="text-[13px] font-semibold text-white/70">Username</Label>
              <div className="relative group">
                ...
                <Input
                  id="admin-username" placeholder="Enter your username" value={username}
            ...
            <div className="space-y-2 animate-slide-up" style={{ animationDelay: "0.1s" }}>
              <Label htmlFor="admin-email" className="text-[13px] font-semibold text-white/70">Email Address</Label>
              <div className="relative group">
                <div className="absolute left-0 top-0 flex h-full w-12 items-center justify-center text-white/30 group-focus-within:text-[#F8CC58]/70 transition-colors duration-300">
                  <Mail className="h-[18px] w-[18px]" />
                </div>
                <Input
                  id="admin-email" type="email" placeholder="Enter your email" value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="h-[52px] pl-12 rounded-[14px] border-white/10 bg-white/[0.04] text-white text-[15px] placeholder:text-white/25 focus:border-[#F8CC58]/50 focus:ring-[#F8CC58]/10 focus:ring-4 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]"
                />
              </div>
